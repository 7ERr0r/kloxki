...
export class _GuiOverlayEquipment extends _Gui {
    ...
    public _centeredHotbarTex(sizex: number, sizey: number, texoffx: number, texoffy: number, offsetx: number) {
        const sc = 2;
    ...
    public _centeredItemTex(tex: _TextureInfo, sizex: number, sizey: number, offsetx: number, offsety: number) {
        const sc = 2;
        const sx = this._klocki._display._width;
        const sy = this._klocki._display._height;
        const cx = (sx >> 1) + offsetx;
